"""The abstract user foundation — defined WITHOUT any concrete model.

``AbstractKeelUser`` carries every field + helper of the extracted user (UUID pk,
...
Importing this module must never define a concrete Django model. That lets a host
project which already has a user table adopt keel-web by simply subclassing
``AbstractKeelUser`` in its own app — keeping its existing ``AUTH_USER_MODEL`` and
table — **without** installing ``keel_web.auth`` as a model app. (Installing it
would register the concrete ``User`` from ``models.py``, whose ``groups`` /
``user_permissions`` reverse accessors would clash with the host's own user on
``auth.Group`` / ``auth.Permission``.) Fresh projects that want keel-web's own
concrete user instead point ``AUTH_USER_MODEL`` at ``keel_web_auth.User``.

Domain models that lived alongside the original user (coupons, purchases) are
intentionally NOT here; they stay in the consuming project.
"""
import uuid
